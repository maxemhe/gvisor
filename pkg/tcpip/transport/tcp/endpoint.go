...
// goroutine.
//
// Each endpoint has a few mutexes.  e.mu -> Primary mutex for an endpoint must
//         be held for all operations except in e.Readiness where acquiring it
//         will result in a deadlock in epoll implementation.
//
...
//
// For more details please see the detailed documentation on e.Lock/Unlock
// methods.
//
// +stateify savable
...
// LockUser tries to lock e.mu and if it fails it will check if the lock is held by
// another syscall goroutine. If yes, then it will goto sleep waiting for the
...
// UnlockUser() will check if there are any segments already queued for
// processing and process any such segments before unlocking e.mu. This is
// required because we when packets arrive and endpoint lock is already held
// then such packets are queued up to be processed. If the lock is held by the
// endpoint goroutine then it will process these packets but if the lock is
// instead held by the syscall goroutine then we can have the syscall goroutine
// process the backlog before unlocking.
//
...
func (e *endpoint) UnlockUser() {
	if e.segmentQueue.empty() {
		if !atomic.CompareAndSwapUint32(&e.ownedByUser, 1, 0) {
