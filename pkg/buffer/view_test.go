...
func testRead(t *testing.T, v *View, n int, wantStr string, wantErr error) {
	t.Helper()
	d := make([]byte, n)
	n, err := v.Read(d)
	if n != len(wantStr) {
...
func TestView(t *testing.T) {
	testCases := []struct {
		...
		// Read & ReadAt.
		{
			...
			input:  "hello",
			output: "",
			op:     func(t *testing.T, v *View) { testRead(t, v, 6, "hello", io.EOF) },
		},
		{
			name:   "read-long",
			input:  "hello",
			output: "",
			op:     func(t *testing.T, v *View) { testRead(t, v, 8, "hello", io.EOF) },
		},
		{
			name:   "read-short",
			input:  "hello",
			output: "lo",
			op:     func(t *testing.T, v *View) { testRead(t, v, 3, "hel", nil) },
		},
		{
			name:   "readat",
			input:  "hello",
